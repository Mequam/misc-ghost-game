...
func tick(player)->void:
	...
	#print("running cider spirit AI for " + str(self.caller.global_position))
	if not self.get_ai_timer().wait_time <= 0 and caller.pressed_inputs["JUMP"] and caller.state == caller.EntityState.DEFAULT:
		...
		if caller.global_position.distance_squared_to(player.global_position) < 600**2 and caller.state != caller.CiderSpiritState.LAUNCHED:
...
func ai_jump_past_player(player,error : float=100)->void:
	if not caller.pressed_inputs["JUMP"]: 
		caller.perform_action("JUMP",true)
	var target = (player.global_position.x - caller.global_position.x)*2
