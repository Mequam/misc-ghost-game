...
func on_col(collider):
	...
	if entity is Entity and not entity is TileMap and \
			entity.collision_layer & (ColMath.Layer.NON_PLAYER_ENTITY | ColMath.Layer.PLAYER) != 0 and \
			entity.can_be_teleported:
